...
//	while((!XUartPsv_IsReceiveData(STDIN_BASEADDRESS)) && (timeout == 0)) // versal
//  while((!XUartPs_IsReceiveData(STDIN_BASEADDRESS)) && (timeout == 0)) // ps_uart
	while (XUartLite_IsReceiveEmpty(STDIN_BASEADDRESS) && (timeout == 0)) // uart_lite
	{
		if ( timeout_ms == 0 )
		...
		{ // no wait - special case
			timeout = 1;
		}
		else if (timeout_ms > 0)
		{
			if (XTmrCtr_GetValue(&TmrCtr, 0) > (timeout_ms * (XPAR_TMRCTR_0_CLOCK_FREQ_HZ / 1000)))
			{
				timeout = 1;
			}
		}
	...
	{
//		c = RecvByte_NonBlocking(); // ps_uart no blocking
//		c = XUartPs_RecvByte(STDIN_BASEADDRESS); // ps_uart blocking
